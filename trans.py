...
config_dir.mkdir(parents=True, exist_ok=True)  # Crée le dossier config si nécessaire

ENV_FILE = config_dir / ".env"
LAST_FOLDER_FILE = config_dir / "last_folder"
...
# Charger les variables depuis le bon .env
load_dotenv(ENV_FILE)  # Modification ici pour charger depuis le chemin absolu
...
def transcribe(file_path: Path, lang: str) -> None:
    ...
    config = Config(
        ...
        verbose=False,
        task='transcribe',
        use_faster_whisper=True,  # Activer faster_whisper
        beam_size=3,  # Réduire la taille du faisceau
        ct2_compute_type='int8',  # Utiliser un type de calcul plus léger
        max_cutting_duration=600,  # Augmenter la durée de découpage
